...
12. 트러블슈팅
  - [1] 정면 거북목 오판정 문제 (Distance Ratio 원리 적용)
    ...
    해결: 목을 앞으로 내밀면 얼굴이 카메라와 가까워져 화면상 귀 사이 거리가 멀어지는 원근법 원리 활용,
          고정 수치 대신 어깨 너비 대비 귀 거리의 상대적 비율(Ratio)로 판정하여 체형/카메라 거리와 무관하게 감지
  ...
  - [2] 거북목/엎드림 오판정 문제 (Vertical Guardrail 도입)
    ...
    해결: 코의 Y축 좌표가 정자세 대비 40px 이상 아래로 내려가면 거북목이 아닌 엎드림으로 판단하는 수직 가드레일 로직 추가,
          두 자세를 명확히 분리하여 정확한 코칭 메시지 전달
